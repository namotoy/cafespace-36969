  ...
  describe '投稿の保存' do
    ...
    context '投稿が保存できない場合' do
      ...
      it '都道府県を選択していないと保存できない' do
        ...
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include("Prefecture を入力してください ")
      end
      ...
      it 'Wifiの情報がないと保存できない' do
        @cafetype.wifi = '0'
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include("Wifi を入力してください ")
      end
      it '電源の情報がないと保存できない' do
        @cafetype.power_supply = '0'
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include("Power supply を入力してください ")
      end
      it '席数の情報がないと保存できない' do
        @cafetype.capacity = '0'
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include("Capacity を入力してください ")
      end
      it 'トイレ場所の情報がないと保存できない' do
        @cafetype.toilet_place = '0'
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include("Toilet place を入力してください ")
      end
      it 'コーヒー価格の情報がないと保存できない' do
        @cafetype.cafe_price = ''
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include("Cafe price を入力してください ")
      end
      it '価格が100円未満では保存できない' do
        @cafetype.cafe_price = '80'
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include('Cafe price  が設定範囲外です')
      end
      ...
      it '価格が全角数字では保存できない' do
        ...
        @cafetype.valid?
        expect(@cafetype.errors.full_messages).to include('Cafe price  が無効です。半角文字で入力してください')
      end
